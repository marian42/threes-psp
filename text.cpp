...
static int charWidth[] = {6, 6, 7, 14, 12, 15, 14, 4, 6, 8, 9, 12, 5, 10, 3, 10, 11, 11, 11, 11, 11, 11, 11, 11, 11, 11, 5, 4, 10, 10, 8, 10, 17, 15, 14, 13, 14, 12, 12, 14, 15, 4, 12, 14, 12, 17, 14, 14, 13, 14, 13, 12, 12, 13, 13, 19, 12, 11, 13, 4, 8, 8, 9, 10, 7, 11, 11, 10, 11, 11, 7, 11, 12, 4, 8, 11, 6, 18, 12, 11, 11, 12, 8, 10, 8, 11, 10, 17, 11, 11, 10, 9, 4, 9};
...
struct GlyphCoordinates {
	...
	GlyphCoordinates(int x, int y, int width, int height) : x ( x ), y ( y ), width ( width ), height ( height ) { } 
};

static const GlyphCoordinates GLYPHS[] = {
...
void drawString(const char* text, int x, int y, unsigned int color, TextAlignment alignment) {
...
void drawGlyph(Glyph glyph, int x, int y) {
	...
	const GlyphCoordinates& glpyhData = GLYPHS[glyph];
    ...
    v0->s = (float)(glpyhData.x);
    v0->t = (float)(glpyhData.y);
    v0->c = 0xFFFFFFFF;
    v0->x = x;
    v0->y = y;
    v0->z = 0.0f;

    v1->s = (float)(glpyhData.x + glpyhData.width);
    v1->t = (float)(glpyhData.y + glpyhData.height);
    v1->c = 0xFFFFFFFF;
    v1->x = x + glpyhData.width;
    v1->y = y + glpyhData.height;
    v1->z = 0.0f;
